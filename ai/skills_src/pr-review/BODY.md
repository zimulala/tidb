...
### 1.4.1 Output Self-Check (Required, Anti-Regression)

When the review task uses a layered output format (for example, review artifacts generated by `wt review`):
- The final output MUST include these section headings:
  - `## 1) Layer 1 - PR Summary`
  - `## 2) Layer 2 - Review Strategy`
  - `## 3) Layer 3 - Detailed Review`
- Before finishing, self-check that all three headings exist and each section is non-empty.
- If any heading/section is missing, regenerate the output and fill missing sections.
- Do NOT finish with findings-only output when layered format is required.
