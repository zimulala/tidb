...
func newAddIndexWorker(decodeColMap map[int64]decoder.Column, bfCtx *backfillCtx, jc *JobContext, jobID, eleID int64, eleTp []byte) (*addIndexWorker, error) {
	...
	if bfCtx.reorgTp == model.ReorgTypeLitMerge {
		...
		if err != nil {
			logutil.BgLogger().Warn("[ddl] new addIndexWorker register failed", zap.Error(err))
...
func (w *addIndexWorker) BackfillDataInTxn(handleRange reorgBackfillTask) (taskCtx backfillTaskContext, errInTxn error) {
	...
	if w.index == nil {
		logutil.BgLogger().Warn(fmt.Sprintf("[ddl] backfill data in txn, w:%#v", w))
	}
...
func (w *worker) addPhysicalTableIndex(t table.PhysicalTable, reorgInfo *reorgInfo) error {
	if reorgInfo.mergingTmpIdx {
		...
		return w.writePhysicalTableRecord(w.sessPool, t, model.TypeAddIndexMergeTmpWorker, reorgInfo)
	}
