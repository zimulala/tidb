...
func (w *worker) onModifyColumn(d *ddlCtx, t *meta.Meta, job *model.Job) (ver int64, _ error) {
	...
	if jp.changingCol == nil {
		...
		newColName := model.NewCIStr(fmt.Sprintf("%s%s", changingColumnPrefix, jp.newCol.Name.O))
		...
		for i, idxInfo := range idxInfos {
			...
			newIdxInfo.Columns[offsets[i]].Offset = jp.newCol.Offset
	...
	return w.doModifyColumnType(d, t, job, dbInfo, tblInfo, jp.changingCol, oldCol, jp.newCol.Name, jp.pos, jp.changingIdxs)
}
