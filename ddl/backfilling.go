...
func (w *backfillWorker) updateLease(exec_id string, bJob *BackfillJob) error {
	...
	bJob.Instance_Lease = GetLeaseGoTime(leaseTime, InstanceLease*time.Second)
...
func (w *backfillWorker) runTask(task *reorgBackfillTask) {
	logutil.BgLogger().Info("[ddl] run backfill worker start", zap.Stringer("worker", w),
		zap.Stringer("task", task), zap.String("bj", task.bfJob.AbbrStr()))
	...
	if result.err == nil {
		...
		task.bfJob.Mate.RowCount = int64(result.addedCount)
		w.finishJob(task.bfJob)
		finish()
	}
	w.resultCh <- result
	logutil.BgLogger().Info("[ddl] run backfill worker finish", zap.Stringer("worker", w), zap.Int("addedCount", result.addedCount))
}
...
func (w *backfillWorker) run(d *ddlCtx, bf backfiller, jobID int64, jobQuery string) {
	...
	for {
		...
		logutil.BgLogger().Debug("[ddl] run backfill worker got task", zap.Stringer("worker", w), zap.String("task", task.String()))
...
func waitTaskResults(workers []*backfillWorker, taskCnt int,
	...
	for i := 0; i < taskCnt; i++ {
		...
		if result.err != nil {
			logutil.BgLogger().Warn("[ddl] backfill worker failed", zap.Stringer("worker", worker), zap.Error(result.err))
		}
...
func (dc *ddlCtx) sendTasksAndWait(sessPool *sessionPool, reorgInfo *reorgInfo, totalAddedCount *int64, workers []*backfillWorker, batchTasks []*reorgBackfillTask) error {
	...
	if err != nil {
		...
		logutil.BgLogger().Warn("[ddl] backfill worker handle batch tasks failed",
			...
			zap.Stringer("takeTime", elapsedTime),
...
func getBatchTasks(t table.Table, reorgInfo *reorgInfo, kvRanges []kv.KeyRange, batchCnt int) []*reorgBackfillTask {
	...
	for i, keyRange := range kvRanges {
		...
		endK, err := getRangeEndKey(reorgInfo.d.jobContext(reorgInfo.Job.ID), reorgInfo.d.store, reorgInfo.Priority, prefix, keyRange.StartKey, endKey)
		if err != nil {
...
func (dc *ddlCtx) writePhysicalTableRecord(sessPool *sessionPool, t table.PhysicalTable, bfWorkerType model.BackfillType, reorgInfo *reorgInfo) error {
	...
	for {
		...
		logutil.BgLogger().Info("[ddl] start backfill workers to reorg record",
			zap.Stringer("worker", backfillWorkers[0]),
			zap.Int("workerCnt", len(backfillWorkers)),
			zap.Int("regionCnt", len(kvRanges)),
			zap.String("startKey", hex.EncodeToString(startKey)),
			zap.String("endKey", hex.EncodeToString(endKey)))
		if bfWorkerType == model.TypeAddIndexBackfill && job.ReorgMeta.ReorgTp == model.ReorgTypeLitMerge {
...
func (dc *ddlCtx) backfillJob2Task(t table.Table, bfJob *BackfillJob) (*reorgBackfillTask, error) {
	pt := t.(table.PhysicalTable)
	if tbl, ok := t.(table.PartitionedTable); ok {
		pt = tbl.GetPartition(bfJob.Mate.PhysicalTableID)
		if pt == nil {
			return nil, dbterror.ErrCancelledDDLJob.GenWithStack("Can not find partition id %d for table %d", bfJob.Mate.PhysicalTableID, t.Meta().ID)
		}
	}
	endKey := bfJob.Mate.EndKey
	// TODO: Check reorgInfo.mergingTmpIdx
	endK, err := getRangeEndKey(dc.jobContext(bfJob.JobID), dc.store, bfJob.Mate.Priority, pt.RecordPrefix(), bfJob.Mate.StartKey, endKey)
	if err != nil {
	...
	return &reorgBackfillTask{
		...
		priority:   bfJob.Mate.Priority}, nil
}
...
func addBatchBackfillJobs(sess *session, bfWorkerType model.BackfillType, reorgInfo *reorgInfo, notDistTask bool,
	batchTasks []*reorgBackfillTask, bJobs []*BackfillJob, id *int64) error {
	bJobs = bJobs[:0]
...
func (dc *ddlCtx) controlWritePhysicalTableRecord(sess *session, t table.PhysicalTable, bfWorkerType model.BackfillType, reorgInfo *reorgInfo) error {
	...
	for {
		...
		if err = addBatchBackfillJobs(sess, bfWorkerType, reorgInfo, notNeedDistProcess, batchTasks, bJobs, &currBackfillJobID); err != nil {
			return errors.Trace(err)
		}
		isFirstOps = false
